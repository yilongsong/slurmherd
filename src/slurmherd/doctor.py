...
def check_cluster(
    ...
    probe_dir = f"{loaded.remote_dir}/.slurmherd-write-test"
    ops = [
...
def detect_site(
    ...
    has_gpu = any(info.get("gpus") for info in partitions.values())
    ...
    lines = [
        ...
        "scheduler: slurm",
        f"gpu_flag: {'gres' if has_gpu else 'gres'}",
        f"account_required: {'true' if accounts and all(r.get('account') for r in accounts) else 'false'}",
        "",
        "resources:",
        "  nodes: 1",
        "  ntasks: 1",
    ]
    default_partition = next(iter(sorted(partitions)), None)
